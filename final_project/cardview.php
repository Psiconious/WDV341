...
if (isset($_POST['submit']) && $admin) {
    ...
    if ($action == 'add') {
        ...
        $sql = "UPDATE yugioh_db_cards SET cardname = :name, cardtype = :cardtype, cardattribute = :attribute, monstertype = :monstertype, monsterlevel = :level, monsteratk = :atk, monsterdef = :def, spelltrapproperty = :stprop, cardeffect = :effect WHERE cardID = :id";
        $stmt = $conn->prepare($sql);
        ...
        try {
            ...
            if ($valid) {
                ...
                header('Location: cardview.php?cardID=' . $id);
            }
        } catch (PDOException $e) {
            $errorMessage .= "<li>Failure to add card to database please try again.</li>";
            if ($e->getCode() == 23000) {
        ...
        $errorMessage = "<ul>";
        $sql = "DELETE FROM yugioh_db_cards WHERE cardID = :id";
        $stmt = $conn->prepare($sql);
        try {
            $stmt->bindParam(':id', $id);
            ...
            $errorMessage .= "<li>Failure to remove entry from database please try again later</li>";
        }
        $errorMessage .= "</ul>";
    } else {
